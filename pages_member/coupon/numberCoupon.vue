	...
	ref="guipage"
		...
		<template v-slot:gBody>
			...
			<view class="list-body" >
				...
				v-for="(coupon, index) in list" :key="index">
					...
					:style="{height:coupon.height}">
						<view class="gui-coupons-left-number">
							<view class="gui-flex gui-row gui-nowrap gui-justify-content-center gui-align-items-center">
								<image :src="coupon.qrcode" style="width: 60px;height: 60px"
								@click="openCode(coupon)"></image>
							</view>
...
export default {
	...
	methods: {
		...
		openCode(e){
			...
			let content = '<div style="font-size: 14px;color: #303133;padding: 10px 20px;text-align: center;">核销二维码</div>' +
			'<img src="'+e.qrcode+'" style="width:150px;height:150px;display:block;margin:5px auto" />';
			this.showModalPopup({
				title:'',//提示标题
				popup_img:'', 
		...
		fetchData() {
			...
			this.$request('memberNumberCouponList', post_data,{
				showLoading: false,
			}).then((res)=>{
