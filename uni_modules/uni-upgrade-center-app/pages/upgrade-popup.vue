		...
		<view class="content botton-radius">
			<view class="content-top">
				<text class="content-top-text">
					{{title}}
					<text style="padding-left:20rpx;font-size: 0.5em;color: #fff;">v.{{version}}</text>
				</text>
			...
			<view class="content-body">
				...
				<view class="body">
					<scroll-view class="box-des-scroll" scroll-y="true">
						<text class="box-des" style="font-size: 14px;color: #303133">
							{{contents}} 
						</text>
					</scroll-view>
				</view>
				<view class="footer flex-center" style="padding-bottom: 30px;">
						...
						<template v-if="!downloadSuccess">
							<view class="progress-box flex-column" v-if="downloading">
								<progress class="progress" :percent="downLoadPercent" activeColor="#3DA7FF" show-info
									stroke-width="10"/>
								<view style="width:100%;font-size: 24rpx;display: flex;justify-content: space-around;padding-top: 5px;">
									<text>{{downLoadingText}}</text>
									<text>({{downloadedSize}}/{{packageFileSize}}M)</text>
								</view>
							</view>
							<button v-if="!is_mandatory" class="content-button" plain @click.stop="closeUpdate">
								暂不更新
							</button>
		...
		methods: {
							...
							if(e.result){
								if(e.result.url){
									...
									downloadTask = uni.downloadFile({
										...
										success: res => {
											if (res.statusCode == 200) {
												t.downloadSuccess = true;
												// fix: wgt 文件下载完成后后缀不是 wgt
												if (t.isWGT && res.tempFilePath.split('.').slice(-1)[0] !== 'wgt') {
									...
									downloadTask.onProgressUpdate(res => {
										t.downLoadPercent = res.progress;
										t.downloadedSize = (res.totalBytesWritten / Math.pow(1024, 2)).toFixed(2);
										t.packageFileSize = (res.totalBytesExpectedToWrite / Math.pow(1024, 2)).toFixed(2);
									});
								}
							}
						},
			...
			installPackage() {
				...
				plus.runtime.install(this.tempFilePath, {
					...
					//console.log('this.installForBeforeFilePath', this.installForBeforeFilePath)
