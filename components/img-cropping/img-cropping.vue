		...
		<view class="oper-wrapper" :style="{ display: sD, top: tp }">
			<view class="oper">
				<view class="btn-wrapper" v-if="sO">
					...
					<view v-if="canSelect" @click="fSelect" :style="{ width: bW }"><text style="color: #ffffff;">重选</text></view>
					<view @click="fRotate" :style="{ width: bW, display: bD }"><text style="color: #ffffff;">旋转</text></view>
					<!-- <view @click="fPreview" hover-class="hover" :style="{width: bW}"><text style="color: #ffffff;">预览</text></view> -->
					<view @click="fUpload" :style="{ width: bW }"><text style="color: #ffffff;">上传</text></view>
				</view>
				<view class="clr-wrapper" v-else>
					<slider
						...
						show-value
					></slider>
					<!-- <view @click="fPrvUpload" :style="{ width: bW }"><text style="color: #ffffff;">上传</text></view> -->
				</view>
			</view>
		</view>
	</view>
...
export default {
	...
	methods: {
		async fWindowResize() {
			// #ifdef APP-PLUS
			let isAndroid = await this.$util.isAndroid();
			if(isAndroid){
				if(!plus.runtime.isAgreePrivacy()) {//用户同意隐私授权
					uni.showToast({
						title:'获取设备信息失败',
						icon:'none'
					})
		...
		async fSelect() {
			...
			let isAndroid = await this.$util.isAndroid();
			if(isAndroid){
				let checkAlbum = await this.$util.showAuthTipModal('android.permission.READ_EXTERNAL_STORAGE');
				if(!checkAlbum){
					return;
				}
				/*let checkCamera = await this.$util.showAuthTipModal('android.permission.CAMERA');
				if(!checkCamera){
					return;
				}*/
			}
